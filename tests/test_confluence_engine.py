...
def test_orderbook_skipped_before_window():
    now = datetime(2026, 7, 24, 9, 10, tzinfo=WIB)   # Friday, 10m after 09:00 open (< 30m window)
    store = ConfluenceStore(":memory:")
    store.upsert_watch("BBRI", news_ts=now.isoformat(), catalyst_text="c", source="portal",
    ...
    out = run_confluence_cycle(
        _settings(), store, now=now, holidays=set(), portal_items=[], disclosures=[],
        daily_data={}, intraday_data={}, auth=object(), sender=_sender_recorder()[1],
        evaluator=lambda *a, **k: [], orderbook_fetcher=ob, is_open=lambda: True)
    assert out == [] and called["n"] == 0            # before window -> fetcher never called
    assert store.get("BBRI").stage_alerted == "2of3"


def test_orderbook_skipped_after_close():
    now = datetime(2026, 7, 24, 16, 30, tzinfo=WIB)  # Friday, past 16:00 close
    store = ConfluenceStore(":memory:")
    store.upsert_watch("BBRI", news_ts=now.isoformat(), catalyst_text="c", source="portal",
                       expires_at="2026-08-01T00:00:00+07:00")
    store.mark_stage_alerted("BBRI", "2of3")
    ...
    def ob(*a, **k):
        called["n"] += 1
        return OrderbookSnapshot(symbol="BBRI", ts=now, total_bid_lot=300000,
                                 total_offer_lot=295000, last_price=4850)
    out = run_confluence_cycle(
        _settings(), store, now=now, holidays=set(), portal_items=[], disclosures=[],
        daily_data={}, intraday_data={}, auth=object(), sender=_sender_recorder()[1],
        evaluator=lambda *a, **k: [], orderbook_fetcher=ob, is_open=lambda: True)
    assert out == [] and called["n"] == 0            # past close -> fetcher never called
    assert store.get("BBRI").stage_alerted == "2of3"


def test_orderbook_non_ready_markup_does_not_upgrade():
    now = datetime(2026, 7, 24, 10, 0, tzinfo=WIB)
    store = ConfluenceStore(":memory:")
    store.upsert_watch("BBRI", news_ts=now.isoformat(), catalyst_text="c", source="portal",
                       expires_at="2026-08-01T00:00:00+07:00")
    store.mark_stage_alerted("BBRI", "2of3")
    snap = OrderbookSnapshot(symbol="BBRI", ts=now, total_bid_lot=300000,
                             total_offer_lot=700000, last_price=4850)  # ratio ~0.43 -> ACCUMULATION
    out = run_confluence_cycle(
        _settings(), store, now=now, holidays=set(), portal_items=[], disclosures=[],
        daily_data={}, intraday_data={}, auth=object(), sender=_sender_recorder()[1],
        evaluator=lambda *a, **k: [], orderbook_fetcher=lambda *a, **k: snap, is_open=lambda: True)
    assert out == []
    assert store.get("BBRI").stage_alerted == "2of3"   # not ready markup -> still waiting


def test_failed_send_leaves_stage_unmarked_for_retry():
    now = datetime(2026, 7, 25, 20, 0, tzinfo=WIB)      # off-hours
    store = ConfluenceStore(":memory:")
    items = [PortalNews(ticker="BBRI", title="t", timestamp=now, url="u", source="s", sentiment="positif")]
    def failing_sender(token, chat_id, text, **kw):
        return False
    out = run_confluence_cycle(
        _settings(), store, now=now, holidays=set(), portal_items=items, disclosures=[],
        daily_data={"BBRI": object()}, intraday_data={}, sender=failing_sender,
        evaluator=lambda *a, **k: [_alert("BBRI", now)], is_open=lambda: False)
    assert out == []
    assert store.get("BBRI").stage_alerted == "none"    # send failed -> not marked -> retries next cycle
    assert store.get("BBRI").breakout_at is not None     # but breakout was recorded
